...
import { X, Check, Star, Zap, CreditCard } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { PlanType, useUserPlan } from '../../hooks/useUserPlan';
import { usePaddle } from '../../hooks/usePaddle';

interface PaddleUpgradeModalProps {
  isOpen: boolean;
  onClose: () => void;
  feature: string;
...
export const PaddleUpgradeModal: React.FC<PaddleUpgradeModalProps> = ({
  isOpen,
  onClose,
  feature,
  currentPlan
}) => {
  const { openCheckout, loading } = usePaddle();
  ...
  const handleUpgrade = async () => {
    const result = await openCheckout('pro');
    if (result?.success) {
      onClose();
    }
  };
              ...
              <div className="p-6">
                ...
                >
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="w-full py-3 px-6 rounded-xl font-semibold transition-all bg-gradient-to-r from-blue-500 to-indigo-600 text-white hover:from-blue-600 hover:to-indigo-700 shadow-lg hover:shadow-xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                  >
                    {loading ? (
